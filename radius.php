    ...
    switch ($action) {
        case 'authenticate':
            ...
            if (!empty($CHAPassword)) {
                $c = ORM::for_table('tbl_customers')->select('password')->select('pppoe_password')->where_raw('BINARY username = ? AND status = ?', [$username, 'Active'])->find_one();
                if ($c) {
                    ...
                    $c = ORM::for_table('tbl_customers')->select('password')->select('pppoe_password')->where_raw('BINARY pppoe_username = ? AND status = ?', [$username, 'Active'])->find_one();
                    if ($c) {
                        if (Password::chap_verify($c['password'], $CHAPassword, $CHAPchallenge)) {
                            $password = $c['password'];
                            $isVoucher = false;
            ...
            if ($username == $password) {
                ...
                $d = ORM::for_table('tbl_voucher')->where_raw('BINARY code = ?', [$username])->find_one();
            } else {
                $d = ORM::for_table('tbl_customers')->where_raw('BINARY username = ? AND status = ?', [$username, 'Active'])->find_one();
        ...
        case 'authorize':
            ...
            if (!empty($CHAPassword)) {
                ...
                if ($c) {
                    ...
                    $c = ORM::for_table('tbl_customers')->select('password')->select('username')->select('pppoe_password')->where_raw('BINARY pppoe_username = ? AND status = ?', [$username, 'Active'])->find_one();
                    if ($c) {
                        if (Password::chap_verify($c['password'], $CHAPassword, $CHAPchallenge)) {
                            $password = $c['password'];
                            $username = $c['username'];
            ...
            $tur = ORM::for_table('tbl_user_recharges')->where_raw('BINARY username = ?', [$username])->find_one();
            if (!$tur) {
                // if check if pppoe_username
                $c = ORM::for_table('tbl_customers')->select('username')->select('pppoe_password')->where_raw('BINARY pppoe_username = ?', [$username])->find_one();
                if ($c) {
                    $username = $c['username'];
                    $tur = ORM::for_table('tbl_user_recharges')->where_raw('BINARY username = ?', [$username])->find_one();
                }
            ...
            if ($tur) {
                if (!$isVoucher && !$isCHAP) {
                    $d = ORM::for_table('tbl_customers')->select('password')->select('pppoe_password')->where_raw('BINARY username = ? AND status = ?', [$username, 'Active'])->find_one();
                    if ($d) {
                        if ($d['password'] != $password) {
                            if ($d['pppoe_password'] != $password) {
                                show_radius_result(['Reply-Message' => 'Username or Password is wrong'], 401);
                        ...
                        $d = ORM::for_table('tbl_customers')->select('password')->select('pppoe_password')->where_raw('BINARY pppoe_username = ? AND status = ?', [$username, 'Active'])->find_one();
                        if ($d) {
                            if ($d['password'] != $password) {
                                if ($d['pppoe_password'] != $password) {
                                    show_radius_result(['Reply-Message' => 'Username or Password is wrong'], 401);
                ...
                if ($isVoucher) {
                    $username = Text::alphanumeric($username, "-_.,");
                    $v = ORM::for_table('tbl_voucher')->where_raw('BINARY code = ? AND routers = ?', [$username, 'radius'])->find_one();
                    if ($v) {
                        if ($v['status'] == 0) {
        ...
        case 'accounting':
            ...
            $d = ORM::for_table('rad_acct')
                ->where_raw('BINARY username = ? AND macaddr = ? AND nasid = ?', [$username, _post('macAddr'), _post('nasid')])
                ->findOne();
            if (!$d) {
            ...
            // pastikan data akunting yang disimpan memang customer aktif phpwifizones
            $tur = ORM::for_table('tbl_user_recharges')->where_raw('BINARY username = ? AND `status` = ? AND `routers` = ?', [$username, 'on', 'radius'])->find_one();
            if (!$tur) {
                // check if pppoe_username
                $c = ORM::for_table('tbl_customers')->select('username')->where_raw('BINARY pppoe_username = ?', [$username])->find_one();
                if ($c) {
                    $username = $c['username'];
                    $tur = ORM::for_table('tbl_user_recharges')->where_raw('BINARY username = ?', [$username])->find_one();
                }
...
function process_radiust_rest($tur, $code)
{
    ...
    $USRon = ORM::for_table('rad_acct')
        ->where_raw('BINARY username = ? AND acctStatusType = ?', [$tur['username'], 'Start'])
        ->find_array();
    // get all the IP
  ...
  // --- Kode pengecekan masa aktif ---
    ...
    if ($plan['typebp'] == "Limited") {
        if ($plan['limit_type'] == "Data_Limit" || $plan['limit_type'] == "Both_Limit") {
            $raddact = ORM::for_table('rad_acct')->where_raw('BINARY username = ?', [$tur['username']])->where('acctstatustype', 'Start')->find_one();
