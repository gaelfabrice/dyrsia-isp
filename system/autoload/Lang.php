...
class Lang
{
    ...
    public static function dateAndTimeFormat($date, $time, array $context = [])
    {
        global $config, $admin;

        if ($context === [] && is_array($admin) && !empty($admin['id'])) {
            $context = ['admin_id' => (int) $admin['id']];
        }

        if (class_exists('WifiZoneTime')) {
            $formatted = WifiZoneTime::formatDisplay((string) $date, (string) $time, $context);
            if ($formatted !== '') {
                return $formatted;
            }
        }
    ...
    {
        global $config;
        return ($config['language'] ?? 'english') === 'french';
    }

    /**
     * Translate activity log text when UI language is French (stored logs + display).
     */
    public static function translateLog($description)
    {
        if (!self::isFrench() || $description === '' || $description === null) {
            return $description;
        }
        ...
        if ($phrases === null) {
            $path = File::pathFixer('system/lan/french_log_phrases.json');
            $phrases = [];
            if (file_exists($path)) {
        ...
        if ($enToFr === null) {
            $enToFr = [];
            $enPath = File::pathFixer('system/lan/english.json');
            $frPath = File::pathFixer('system/lan/french.json');
            if (file_exists($enPath) && file_exists($frPath)) {
                ...
                foreach ($en as $k => $v) {
                    ...
                    if (!empty($fr[$k]) && $fr[$k] !== $v) {
                        $enToFr[$v] = $fr[$k];
                    }
                }
                uksort($enToFr, function ($a, $b) {
                    return strlen($b) - strlen($a);
                });
            }
        }
        ...
        foreach ($phrases as $en => $fr) {
            $out = str_replace($en, $fr, $out);
        }
        foreach ($enToFr as $en => $fr) {
            if (strpos($out, $en) !== false) {
                $out = str_replace($en, $fr, $out);
            }
        }
  ...
  /**
     ...
     * @return array|object[]
     */
    public static function translateLogRows($rows)
    {
        if (!self::isFrench() || empty($rows)) {
            return $rows;
        }
        foreach ($rows as $i => $row) {
            if (is_object($row) && isset($row->description)) {
                $row->description = self::translateLog($row->description);
            } elseif (is_array($row) && isset($row['description'])) {
                $rows[$i]['description'] = self::translateLog($row['description']);
            }
        }
        return $rows;
    }
