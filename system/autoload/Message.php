...
class Message
{
    ...
    public static function sendEmail($to, $subject, $body, $attachmentPath = null, $rawHtml = false)
    {
        ...
        if (empty($config['smtp_host'])) {
            ...
            if ($rawHtml) {
                $attr .= "MIME-Version: 1.0\r\n";
                $attr .= "Content-Type: text/html; charset=UTF-8\r\n";
            }
            $sent = mail($to, $subject, $body, $attr);
            self::logMessage('Email', $to, $body, $sent ? 'Success' : 'Error', $sent ? '' : 'PHP mail() failed');
            return $sent;
        } else {
            $mail = new PHPMailer();
            $mail->isSMTP();
            ...
            if (!file_exists($PAGES_PATH . DIRECTORY_SEPARATOR . 'Email.html')) {
                if (!copy($PAGES_PATH . '_template' . DIRECTORY_SEPARATOR . 'Email.html', $PAGES_PATH . DIRECTORY_SEPARATOR . 'Email.html')) {
                    file_put_contents($PAGES_PATH . DIRECTORY_SEPARATOR . 'Email.html', Http::getData('https://raw.githubusercontent.com/hotspotbilling/phpwifizones/master/pages_template/Email.html'));
                }
            }
            ...
            } else if (file_exists($PAGES_PATH . DIRECTORY_SEPARATOR . 'Email.html')) {
