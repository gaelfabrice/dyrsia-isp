...
class Admin
{
    ...
    public static function getID()
    {
        ...
        if ($enable_session_timeout && !empty($_SESSION['aid']) && !empty($_SESSION['aid_expiration'])) {
            if ($_SESSION['aid_expiration'] > time()) {
                ...
                return self::ensureActiveAccount((int) $_SESSION['aid']);
            } else {
        ...
        elseif (isset($_COOKIE['aid'])) {
            ...
            if (WifiZoneSecurity::verifyCookieSignature($tmp)) {
                ...
                if (!empty($_COOKIE['aid']) && !$isValid) {
                    ...
                    if (time() - $tmp[1] < 86400 * 7) {
                        ...
                        return self::ensureActiveAccount((int) $tmp[0]);
                    }
                }
            }
        }
    ...
    public static function revokeSessions($aid)
    {
        $aid = (int) $aid;
        if ($aid <= 0) {
            return;
        }
        $user = ORM::for_table('tbl_users')->find_one($aid);
        if ($user) {
    ...
    private static function ensureActiveAccount($aid)
    {
        if ($aid <= 0) {
            return 0;
        }
        ...
        if (!$user || $user->status !== 'Active') {
            self::removeCookie();
            _alert(Lang::T('This account status') . ' : ' . Lang::T('Inactive'), 'danger', "admin");
            return 0;
        }
        return $aid;
    }
