...
function home_scoped_router_query($user)
{
    $query = ORM::for_table('tbl_routers');
    if (!empty($user['created_by'])) {
        $query->where('admin_id', $user['created_by']);
    }
    return $query;
}
...
if (_post('send') == 'balance') {
    ...
    foreach ($actives as $active) {
        $router = home_scoped_router_query($user)->where('name', $active['routers'])->find_one();
        if ($router) {
        ...
        if ($tur['status'] == 'on') {
            ...
            if ($p) {
                ...
                if ($c) {
                    ...
                    $planName = $p['name_plan'] ?? $tur['namebp'];
                    $log .= "DONE : $planName, $tur[type], $tur[routers]<br>";
                } else {
...
if (isset($_GET['recharge']) && !empty($_GET['recharge'])) {
    ...
    if ($bill) {
        if ($bill['routers'] == 'radius') {
            ...
            $routers = home_scoped_router_query($user)->where('name', $bill['routers'])->find_one();
            $router = $routers['id'];
        }
...
if (!empty($_SESSION['nux-mac']) && !empty($_SESSION['nux-ip']) && ($_c['hs_auth_method'] ?? '') != 'hchap') {
    ...
    if ($bill) {
        $p = ORM::for_table('tbl_plans')->where('id', $bill['plan_id'])->find_one();
        $dvc = Package::getDevice($p);
        if ($_app_stage != 'demo' && $p && file_exists($dvc)) {
            require_once $dvc;
            $mikrotik = _get('mikrotik');
            if ($mikrotik == 'login') {
