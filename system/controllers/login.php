...
switch ($do) {
    case 'post':
        ...
        if ($username != '' and $password != '') {
            if (!WifiZoneSecurity::enforceLoginRateLimit('customer_login', $username)) {
                _log($username . ' ' . Lang::T('Failed Login') . ' [rate limit]', 'User');
                _msglog('e', Lang::T('Too_many_attempts__Please_try_again_later_'));
                r2(getUrl('portal'));
            }
            $d = ORM::for_table('tbl_customers')->where('username', $username)->find_one();
            if (!$d && Validator::Email($username)) {
                $d = ORM::for_table('tbl_customers')->where('email', $username)->find_one();
            }
            if ($d) {
                $d_pass = $d['password'];
                if ($d['status'] == 'Banned') {
                ...
                if (!in_array($d['status'], ['Active', 'Limited'], true)) {
                    _msglog('e', Lang::T('This account status') . ' : ' . Lang::T($d['status']));
                    r2(getUrl('portal'));
                }
                if (Password::verifyCustomer($password, $d_pass)) {
    ...
    default:
        ...
        if ($config['disable_registration'] == 'noreg') {
