...
switch ($action) {
    ...
    case 'change-password-post':
        ...
        if ($password != '') {
            ...
            if ($password == $d_pass) {
                ...
                User::removeCookie();
                session_destroy();
                _log('[' . $user['username'] . ']: ' . Lang::T('Password changed successfully'), 'User', $user['id']);
    ...
    case 'edit-profile-post':
        ...
        if (empty($msg)) {
            File::handleUserPhotoUpload($user, $UPLOAD_PATH);
    ...
    case 'language-update-post':
        $selected_language = strtolower(_req('lang', 'english'));
        if (wifizone_apply_language($selected_language)) {
            $label = $selected_language === 'french' ? 'Français' : 'English';
            r2($_SERVER['HTTP_REFERER'] ?: getUrl('home'), 's', Lang::T('Language') . ': ' . $label);
        }
        r2($_SERVER['HTTP_REFERER'] ?: getUrl('home'), 'e', Lang::T('Invalid or Expired CSRF Token') . '.');
        break;
