        ...
        <form method="get" class="form">
            <div class="box box-primary box-solid">
                ...
                <div id="filter_box" class="box-body hidden-xs hidden-sm hidden-md">
                    ...
                    <input type="date" class="form-control" name="sd" value="{$sd|date_format:'%Y-%m-%d'}">
                    <label>{Lang::T('Start time')}</label>
                    <input type="time" class="form-control" name="ts" value="{$ts}">
                    <label>{Lang::T('End Date')}</label>
                    <input type="date" class="form-control" name="ed" value="{$ed|date_format:'%Y-%m-%d'}">
                    ...
                    <label>{Lang::T('Methods')} / {Lang::T('Processed By')}</label>
<select class="form-control select2" name="mts[]" multiple>
    <!-- ১. কোর পেমেন্ট মেথডস (আগের মতই থাকবে) -->
    ...
    <!-- ২. অ্যাডমিন এবং রিসেলারদের ফুল নেম (নতুন যোগ করা হয়েছে) -->
    <optgroup label="Admin / Resellers">
        {foreach $all_staff as $staff}
            <option value="{$staff['username']}" {if in_array($staff['username'], $mts)}selected{/if}>
                {$staff['fullname']}
            </option>
        {/foreach}
    </optgroup>
