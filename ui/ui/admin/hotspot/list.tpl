    ...
    <div class="col-sm-12">
        <div class="panel panel-hovered mb20 panel-primary">
            ...
            <div class="table-responsive">
                <form id="hotspotBulkForm" method="post" action="{Text::url('services/hotspot-bulk-delete')}">
                    <div style="margin-left: 5px; margin-right: 5px;">
                        <table class="table table-bordered table-striped table-condensed">
                            <thead>
                                <tr>
                                    <th width="30"><input type="checkbox" id="checkAll"></th>
                                    <th colspan="6" class="text-center">{Lang::T('Internet Package')}</th>
                            ...
                            <tbody>
                                {foreach $d as $ds}
                                    <tr {if $ds['enabled'] !=1}class="danger" title="disabled" {elseif $ds['prepaid'] !='yes'}class="warning" title="Postpaid" {/if}>
                                        ...
                                        <td>{$ds['device']}</td>
                                        <td>{if $ds['plan_expired']}<a href="{Text::url('services/edit/')}{$ds['plan_expired']}">{Lang::T('Yes')}</a>{else}{Lang::T('No')}{/if}</td>
                                        <td>{if $ds['prepaid'] == no}{$ds['expired_date']}{/if}</td>
                                        <td>{$ds['id']}</td>
                                        <td>
                                            <a href="{Text::url('services/edit/')}{$ds['id']}" class="btn btn-info btn-xs">{Lang::T('Edit')}</a>
                                        </td>
                                    </tr>
                                {/foreach}
                            </tbody>
    ...
    if (typeof Swal !== "undefined") {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                document.getElementById('hotspotBulkForm').submit();
            }
        });
    } else {
